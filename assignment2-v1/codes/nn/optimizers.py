...
class SGD(Optimizer):
    def __init__(
        self, lr=0.01, momentum=0, decay=0, nesterov=False, sheduler_func=None
    ):
        """Initialization
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            self.moments[k] = (
                self.momentum * self.moments[k] - self.lr * xs_grads[k]
            )
            if self.nesterov:
                new_xs[k] = (
                    xs[k]
                    - self.momentum * prev_moments[k]
...
class RMSprop(Optimizer):
    def __init__(
        self, lr=0.001, rho=0.9, epsilon=None, decay=0, sheduler_func=None
    ):
        """Initialization
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            self.accumulators[k] = (
                self.rho * self.accumulators[k]
                + (1 - self.rho) * xs_grads[k] ** 2
            )
            new_xs[k] = xs[k] - self.lr * xs_grads[k] / (
                np.sqrt(self.accumulators[k]) + self.epsilon
            )
...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            self.moments[k] = (
                self.beta_1 * self.moments[k] + (1 - self.beta_1) * xs_grads[k]
            )
            self.accumulators[k] = (
                self.beta_2 * self.accumulators[k]
                + (1 - self.beta_2) * xs_grads[k] ** 2
            )
            if self.bias_correction:
                moments_t = self.moments[k] / (
                    1 - self.beta_1 ** (iteration + 1)
                )
