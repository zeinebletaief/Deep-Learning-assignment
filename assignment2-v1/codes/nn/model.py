...
class Model:
        ...
        for l, layer in enumerate(self.layers):
            if layer.trainable:
                ...
                for k in layer_params.keys():
                    ...
                    assert ~np.any(
                        np.isnan(layer_params[k])
                    ), "{} contains NaN".format(k)
                layer.update(layer_params)
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for iteration in range(num_train // train_batch):
                total_iteration = (
                    epoch * (num_train // train_batch) + iteration
                )
                # output test loss and accuracy
                if (
                    test_intervals > 0
                    and iteration > 0
                    and iteration % test_intervals == 0
                ):
                ...
                if iteration > 0 and iteration % print_intervals == 0:
                    ...
                    print(
                        "Train iter %d/%d:\t"
                        % (iteration, num_train // train_batch),
                        end="",
                    )
    ...
    def val(self, dataset, val_batch):
        ...
        try:
            ...
            print("Val accuracy %.2f, loss %.2f" % (accuracy, avg_loss))
